...
export async function createProduct(prevState: any, formData: FormData) {
  ...
  if (!title || !priceRaw || !categoryId) {
    return { error: "Missing required fields: Title, Price, and Category are required." };
  }
  ...
  if (salePriceRaw) {
    ...
    if (salePrice >= price) {
      return { error: "Sale price must be strictly less than the regular price." };
    }
  }
  ...
  if (file && file.size > 0 && file.name) {
    ...
    if (uploadError) {
      ...
      return { error: `Failed to upload image to storage: ${uploadError.message}` };
    }
...
export async function deleteProduct(productId: string) {
  ...
  if (product?.image_url && product.image_url.includes("/storage/v1/object/public/product-images/")) {
    try {
      ...
      console.error("Failed to cleanup storage image on product delete:", cleanupError);
    }
  }
...
export async function updateOrderStatus(orderId: string, status: "shipped" | "completed") {
  const isAdmin = await checkIsAdmin();
  if (!isAdmin) {
    return { error: "Unauthorized. Admin access required." };
  }
  ...
  const title = status === "shipped" ? "Order Shipped! 🚚" : "Order Arrived! 📦";
